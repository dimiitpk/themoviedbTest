...
import com.dimi.themoviedb.ui.main.movies.adapters.MovieListAdapter
...
class MovieFragment : BaseMovieFragment(),
    ...
    override fun onItemSelected(position: Int, item: Movie) {
        viewModel.setClickedMovieId(item.id)
        findNavController().navigate(R.id.action_movieFragment_to_viewMovieFragment)
    }

    override fun restoreListPosition() {
        viewModel.viewState.value?.movieFields?.layoutManagerState?.let { lmState ->
    ...
    private fun setupTabs() {
        ...
        for (genre in MovieGenres.getAllGenreIds()!!) {
            val newTab = movie_tabs.newTab()
            newTab.text = MovieGenres.getGenreName(genre);
    ...
    override fun onTabReselected(tab: TabLayout.Tab) {
        ...
        if (tab.position == 0) {
            // if popular is reselect and some query was called it will be restarted back to popular movie query
    ...
    override fun onTabSelected(tab: TabLayout.Tab) {
        ...
        if (tab.position != 0) {
            //viewModel.setQuery("")
            viewModel.setGenre(MovieGenres.getGenreIdByName(tab.text as String?))
        } else
            viewModel.clearGenre()
